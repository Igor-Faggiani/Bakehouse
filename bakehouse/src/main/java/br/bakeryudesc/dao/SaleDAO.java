...
public class SaleDAO extends AbstractDAO<Sale, Long> {
    ...
    public List<Sale> findByCustomerId(Long customerId) {
        TypedQuery<Sale> query = entityManager.createQuery(
                "SELECT s FROM Sale s WHERE s.customer.id = :customerId ORDER BY s.saleDate DESC", Sale.class);
    ...
    public List<Sale> findByDateRange(LocalDateTime start, LocalDateTime end) {
        TypedQuery<Sale> query = entityManager.createQuery(
                "SELECT s FROM Sale s WHERE s.saleDate BETWEEN :start AND :end ORDER BY s.saleDate DESC", Sale.class);
